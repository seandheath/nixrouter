...
select_interface() {
    ...
    mapfile -t interfaces < <(list_interfaces | grep -v "^${exclude}$")
...
# LAN: connects to local network/switch
...
main() {
    ...
    lan=$(select_interface "Select LAN interface (connects to local network):" "$wan")
    success "LAN: $lan"

    # Write interfaces.nix
    write_interfaces "$wan" "$lan"
    ...
    echo "  3. Connect LAN ($lan) to your switch/network"
    echo "  4. SSH to admin@10.0.0.1 from the LAN"
    echo ""
